  ...
  useEffect(() => {
    ...
    virtualRafRef.current = requestAnimationFrame(() => {
      try {
        ...
        const fromSec = fromTime != null ? toEpochSec(fromTime) : null;
        const toSec = toTime != null ? toEpochSec(toTime) : null;
  ...
  useEffect(() => {
    ...
    const onTimeRange = (range) => {
      ...
      if (!shouldExpand) {
        ...
        const hasLoadMoreFn = typeof loadMoreHistory === 'function';
        ...
        try {
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.debug(
              ...
              { first: nextSlice[0], last: nextSlice[nextSlice.length - 1] },
            );
          }
          ...
          console.log('%c[ЧАРТ ОТРИСОВАЛ]', 'color: #00ff44; font-size: 16px; font-weight: bold;', {
            ...
            интервал: currentInterval,
            первая_дата: nextSlice?.[0] ? new Date(nextSlice[0].time * 1000).toLocaleDateString('ru-RU') : '—',
            последняя_дата: nextSlice?.length ? new Date(nextSlice[nextSlice.length-1].time * 1000).toLocaleDateString('ru-RU') : '—',
          });

          const t1 = performance.now();
          console.log('[LWC] vr setData', nextSlice.length, 'bars |', (t1 - t0).toFixed(1), 'ms');

          virtualRangeRef.current = {
            fromIdx,
            toIdx,
            fromTime: fullDataRef.current[fromIdx]?.time ?? null,
            toTime: fullDataRef.current[toIdx]?.time ?? null,
          };
